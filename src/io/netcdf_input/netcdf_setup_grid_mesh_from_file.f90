module netcdf_setup_grid_mesh_from_file
  ...
  use grid_orca_basic, only: calc_orca_field_to_vector_form_translation_tables
  ...
  public :: setup_xy_grid_from_file, setup_lonlat_grid_from_file, setup_orca_grid_from_file, setup_mesh_from_file, &
...
contains
  ...
  subroutine setup_orca_grid_from_file( filename, ncid, grid)
    ...
    character(len=*),       intent(in   ) :: filename
    integer,                intent(in   ) :: ncid
    type(type_grid_orca),   intent(  out) :: grid
    ...
    ! Secondary data
    ! call calc_orca_field_to_vector_form_translation_tables( grid)

    ! Finalise routine path
    call finalise_routine( routine_name)

  end subroutine setup_orca_grid_from_file
