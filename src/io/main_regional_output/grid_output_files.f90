...
contains
  ...
  subroutine write_to_main_regional_output_file_grid_field( region, grid, filename, ncid, choice_output_field)
    ...
    ! ============
      ...
      case ('H_lad')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%now%H, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'H_lad', d_grid_vec_partial_2D)
      case ('U_lad')
        call map_from_mesh_triangles_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%now%U, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'U_lad', d_grid_vec_partial_2D)
      case ('V_lad')
        call map_from_mesh_triangles_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%now%V, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'V_lad', d_grid_vec_partial_2D)
      case ('T_lad')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%now%T, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'T_lad', d_grid_vec_partial_2D)
      case ('S_lad')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%now%S, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'S_lad', d_grid_vec_partial_2D)

      ! Useful laddie fields
      case ('drho_amb')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%drho_amb, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'drho_amb', d_grid_vec_partial_2D)
      case ('drho_base')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%drho_base, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'drho_base', d_grid_vec_partial_2D)
      case ('entr')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%entr, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'entr', d_grid_vec_partial_2D)
      case ('entr_dmin')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%entr_dmin, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'entr_dmin', d_grid_vec_partial_2D)
      case ('melt')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%melt, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'melt', d_grid_vec_partial_2D)
      case ('divQH')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%divQH, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'divQH', d_grid_vec_partial_2D)
      case ('divQT')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%divQT, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'divQT', d_grid_vec_partial_2D)
      case ('divQS')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%divQS, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'divQS', d_grid_vec_partial_2D)
      case ('diffT')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%diffT, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'diffT', d_grid_vec_partial_2D)
      case ('diffS')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%diffS, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'diffS', d_grid_vec_partial_2D)
      case ('viscU')
        call map_from_mesh_triangles_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%viscU, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'viscU', d_grid_vec_partial_2D)
      case ('viscV')
        call map_from_mesh_triangles_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%viscV, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'viscV', d_grid_vec_partial_2D)
      case ('T_base')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%T_base, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'T_base', d_grid_vec_partial_2D)
      case ('T_amb')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%T_amb, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'T_amb', d_grid_vec_partial_2D)
      case ('u_star')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%u_star, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'u_star', d_grid_vec_partial_2D)
      case ('gamma_T')
        call map_from_mesh_vertices_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%gamma_T, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'gamma_T', d_grid_vec_partial_2D)
      case ('divQU')
        call map_from_mesh_triangles_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%divQU, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
        call write_to_field_multopt_grid_dp_2D( grid, filename, ncid, 'divQU', d_grid_vec_partial_2D)
      case ('divQV')
        call map_from_mesh_triangles_to_xy_grid_2D( region%mesh, grid, region%BMB%laddie%divQV, d_grid_vec_partial_2D, d_mesh_is_hybrid = .true.)
