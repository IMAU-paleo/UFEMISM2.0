module grid_types
  ...
  type type_grid_orca
    ! An ORCA or eORCA grid as used in the NEMO ocean model
    ! This is a x,y grid with irregular longitude-latitude values attributed to each grid point

    ! Basic properties
    character(len=256)                      :: name         !           A nice name tag
    integer                                 :: nx           !           Number of grid cells in the x-direction
    integer                                 :: ny           !           Number of grid cells in the y-direction
    integer                                 :: n            !           Total number of grid cells (= nx * ny)
    real(dp), dimension(:,:  ), allocatable :: lon          ! [degrees east ] Longitude of each grid point
    real(dp), dimension(:,:  ), allocatable :: lat          ! [degrees north] Latitude  of each grid point
    real(dp)                                :: xmin         ! []        x and y range covered by the grid
    real(dp)                                :: xmax         ! []
    real(dp)                                :: ymin         ! []
    real(dp)                                :: ymax         ! []
