...
! =======================
  ...
  SUBROUTINE solve_SIA( mesh, ice, SIA)
    ...
    DO ti = 1, mesh%nTri_loc
      ...
      int_A_hminzetan = integrate_from_zeta_is_one_to_zeta_is_zetap( z, A_flow_3D_b( ti,:) * (Hs_b( ti) - z)**ice%C%n_flow)

      ! Calculate the diffusivity term
      abs_grad_Hs = SQRT( dHs_dx_b( ti)**2 + dHs_dy_b( ti)**2)
      SIA%D_3D_b( ti,:) = -2._dp * (ice_density * grav)**ice%C%n_flow * abs_grad_Hs**(ice%C%n_flow - 1._dp) * int_A_hminzetan

      ! Safety
      SIA%D_3D_b( ti,:) = MAX( -ice%C%SIA_maximum_diffusivity, SIA%D_3D_b( ti,:))
    ...
    DO vi = 1, mesh%nV_loc
      ...
      DO k = 1, mesh%nz
        SIA%du_dz_3D_a( vi,k) = -2._dp * (ice_density * grav)**ice%C%n_flow * abs_grad_Hs**(ice%C%n_flow - 1._dp) * &
          ice%A_flow_3D( vi,k) * (ice%Hs( vi) - z( k))**ice%C%n_flow * dHs_dx_a( vi)
        SIA%dv_dz_3D_a( vi,k) = -2._dp * (ice_density * grav)**ice%C%n_flow * abs_grad_Hs**(ice%C%n_flow - 1._dp) * &
          ice%A_flow_3D( vi,k) * (ice%Hs( vi) - z( k))**ice%C%n_flow * dHs_dy_a( vi)
      END DO
