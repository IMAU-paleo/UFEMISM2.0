module nudging_utilities
  ...
  public :: calc_nudging_vs_extrapolation_masks, trace_flowline_upstream, trace_flowline_downstream, &
    calc_half_flowline_average, map_field_from_vertices_to_flowline
...
contains
  ...
  subroutine map_field_from_vertices_to_flowline( mesh, d_tot, &
    vi, T, n, d_flowline)
    ...
    integer,                      intent(in   ) :: n
    real(dp), dimension(mesh%nV), intent(  out) :: d_flowline
    ...
    do k = 1, n
      p = T( k,:)
      call interpolate_to_point_dp_2D_singlecore( mesh, d_tot, p, ti, d_flowline( k))
    end do

  end subroutine map_field_from_vertices_to_flowline
