module bed_roughness_nudging_H_dHdt_flowline
  ...
  use nudging_utilities, only: calc_nudging_vs_extrapolation_masks, trace_flowline_upstream, &
    trace_flowline_downstream, map_field_from_vertices_to_flowline, calc_half_flowline_average
  ...
  subroutine calc_flowline_averaged_deltaHs_dHsdt( mesh, ice, target_geometry, nudge)
    ...
    do vi = mesh%vi1, mesh%vi2
      ...
      if (nudge%mask_calc_dCdt_from_nudging( vi)) then
        ...
        call map_field_from_vertices_to_flowline( mesh, deltaHs_tot, vi, trace_up  , n_up  , deltaHs_up  )
        call map_field_from_vertices_to_flowline( mesh, deltaHs_tot, vi, trace_down, n_down, deltaHs_down)
        call map_field_from_vertices_to_flowline( mesh, dHs_dt_tot , vi, trace_up  , n_up  , dHs_dt_up   )
        call map_field_from_vertices_to_flowline( mesh, dHs_dt_tot , vi, trace_down, n_down, dHs_dt_down )
