...
! ====================
  ...
  use mpi_f08, only: MPI_WIN, mpi_allreduce, mpi_in_place, mpi_integer, mpi_sum, mpi_comm_world
...
! =========================
  ...
  SUBROUTINE integrate_fbrk3( mesh, ice, ocean, laddie, tl, time, dt)
    ...
    call allocate_dist_shared( Hstar, wHstar, mesh%nV_node)
    Hstar( mesh%vi1_node:mesh%vi2_node) => Hstar
    ...
    call deallocate_dist_shared( Hstar, wHstar)
  ...
  SUBROUTINE extrapolate_laddie_variables( mesh, ice, laddie)
    ...
    ! Allocate hybrid distributed/shared memory
    call allocate_dist_shared( mask, wmask, mesh%nV_node)
    mask( mesh%vi1_node:mesh%vi2_node) => mask

    ! Initialise mask
    mask( mesh%vi1:mesh%vi2) = 0
    ...
    CALL extrapolate_Gaussian( mesh, mask, laddie%now%H, sigma, d_is_hybrid = .true.)
    CALL extrapolate_Gaussian( mesh, mask, laddie%now%T, sigma, d_is_hybrid = .true.)
    CALL extrapolate_Gaussian( mesh, mask, laddie%now%S, sigma, d_is_hybrid = .true.)
    ...
    ! Clean up after yourself
    call deallocate_dist_shared( mask, wmask)
