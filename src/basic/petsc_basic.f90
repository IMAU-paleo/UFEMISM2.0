...
#include <petsc/finclude/petscksp.h>
  ...
  USE mpi_distributed_memory                                 , ONLY: partition_list, allgather_array, gather_to_all_dp_1D
...
! == Conversion between 1-D Fortran double-precision arrays and PETSc parallel vectors
  ...
  SUBROUTINE mat_CSR2petsc( AA, A)
    ...
    INTEGER                                            :: nrows_proc, i, k1, k2, nnz_proc, ii, k, kk
    ...
    END DO
    ! Last row
    ptr_proc( nrows_proc) = AA%ptr( AA%i2+1) - AA%ptr( AA%i1)

    ! Create PETSc matrix
!    IF (AA%balanced) then
!      CALL MatCreateMPIAIJWithArrays( PETSC_COMM_WORLD, nrows_proc, PETSC_DECIDE, AA%m, AA%n, ptr_proc, ind_proc, val_proc, A, perr)
!    ELSE
...
! == Matrix-vector multiplication
  SUBROUTINE multiply_CSR_matrix_with_vector_1D( AA, xx, yy)
    ...
    !
    ! NOTE: both A and x are stored as distributed memory
