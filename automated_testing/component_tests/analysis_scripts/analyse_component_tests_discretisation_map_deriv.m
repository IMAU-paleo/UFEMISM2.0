...
function [H, max_errs] = analyse_mapping_derivatives_tests_mesh_function_map( filename)
  ...
  if do_print_figures
    ...
    title( H.Ax{ 1,5},'Discretisation error')
    ...
    xlabel( H.Ax{ 1,4},'from A')
    xlabel( H.Ax{ 1,5},'from B')
  
    ylabel( H.Ax{ 1,4},'to A')
    ylabel( H.Ax{ 2,4},'to B')
  ...
  if do_print_figures
    ...
    % plot_mesh_data_a( H.Ax{ 1,4}, mesh, err_a_a, clim_err);
    plot_mesh_data_a( H.Ax{ 1,5}, mesh, err_b_a, clim_err);
    ...
    plot_mesh_data_b( H.Ax{ 2,4}, mesh, err_a_b, clim_err);
    % plot_mesh_data_b( H.Ax{ 2,5}, mesh, err_b_b, clim_err);
  
    %% Add colorbar for errors
    pos = get( H.Ax{ 1,5},'position');
    x_left  = pos( 1) + pos( 3) + 5;
    y_top   = pos( 2) + pos( 4);
    pos = get( H.Ax{ 2,5},'position');
    y_bot   = pos( 2);
    ha = y_top - y_bot;
    ...
    colormap( H.Ax_cbar, colormap( H.Ax{ 1,4}));
