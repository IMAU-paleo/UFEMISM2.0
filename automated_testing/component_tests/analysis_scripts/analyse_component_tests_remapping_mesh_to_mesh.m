...
function analyse_remapping_test( filename_short)
  ...
  mesh1.TriA     = ncread( filename_full, 'mesh1_TriA');
  ...
  d_mesh1_b_ex   = ncread( filename_full, 'd_mesh1_b_ex');
  d_mesh2_b_ex   = ncread( filename_full, 'd_mesh2_b_ex');
  ...
  if do_print_figures
    ...
    wa = 300;
    ha = wa;
    H = setup_multipanel_figure( wa, ha, [5,5,5,5,100], [5, 5]);

    for j = 1:size(H.Ax,2)
      set( H.Ax{1,j},'xtick',[],'ytick',[],'xgrid','off','ygrid','off')
      H.Ax{1,j}.XAxis.Visible = 'off';
      H.Ax{1,j}.YAxis.Visible = 'off';
    end

    H.mesh_nn     = patch('parent',H.Ax{1,1},'vertices',mesh2.V,'faces',mesh2.Tri,...
      'facevertexcdata',d_err_nn,    'facecolor','interp','edgecolor','none');
    H.mesh_trilin = patch('parent',H.Ax{1,2},'vertices',mesh2.V,'faces',mesh2.Tri,...
      'facevertexcdata',d_err_trilin,'facecolor','interp','edgecolor','none');
    H.mesh_cons   = patch('parent',H.Ax{1,3},'vertices',mesh2.V,'faces',mesh2.Tri,...
      'facevertexcdata',d_err_cons,  'facecolor','interp','edgecolor','none');
    H.mesh_b_cons = patch('parent',H.Ax{1,4},'vertices',mesh2.V,'faces',mesh2.Tri,...
      'facevertexcdata',d_err_b_cons,'facecolor','flat',  'edgecolor','none');
    ...
    H.Cbaraxes = axes('parent',H.Fig,'units','pixels','position',[x_lo,y_lo,wa,ha],...
      'fontsize',get(H.Ax{1,4},'fontsize'));
    ...
    for j = 1: size( H.Ax,2)
      colormap( H.Ax{1,j},cmap);
      set( H.Ax{1,j},'clim',clim);
    end
  ...
  write_to_scoreboard_file( filename_short, mesh1, d_mesh1_ex, d_mesh1_b_ex, mesh2, d_mesh2_ex, d_mesh2_b_ex, ...
    d_mesh2_nn, d_mesh2_trilin, d_mesh2_cons, d_mesh2_b_cons)
  
end

  function write_to_scoreboard_file( filename_short, mesh1, d_mesh1_ex, d_mesh1_b_ex, mesh2, d_mesh2_ex, d_mesh2_b_ex, ...
    d_mesh2_nn, d_mesh2_trilin, d_mesh2_cons, d_mesh2_b_cons)
  ...
  rmse_b_cons = sqrt( mean( (d_mesh2_b_cons - d_mesh2_b_ex).^2));
  ...
  bounds_max_b_cons = max( 0, max( d_mesh2_b_cons(:)) - max( d_mesh1_b_ex(:)));
  ...
  int_err_b_cons = abs( 1 - int_mesh2_b_cons / int_mesh1_b);
  ...
  single_run = add_cost_function_to_single_run( single_run, 'rmse_b_cons'       , 'sqrt( mean( (d_mesh2_b_cons - d_mesh2_b_ex).^2))'      , rmse_b_cons);
  ...
  single_run = add_cost_function_to_single_run( single_run, 'bounds_max_b_cons' , 'max( 0, max( d_mesh2_b_cons(:)) - max( d_mesh1_b_ex(:)))', bounds_max_b_cons);
  ...
  single_run = add_cost_function_to_single_run( single_run, 'bounds_min_b_cons' , 'max( 0, min( d_mesh2_b_ex(:)) - min( d_mesh2_b_cons(:)))', bounds_min_b_cons);
  ...
  single_run = add_cost_function_to_single_run( single_run, 'int_err_b_cons'    , 'abs( 1 - int_mesh2_b_cons / int_mesh1_b)'              , int_err_b_cons);
