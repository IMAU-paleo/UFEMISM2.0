function analyse_component_tests_remapping_mesh_triangles_to_grid( foldername_automated_testing, do_print_figures)
% Analyse the results of all the mesh-to-grid remapping component tests

disp('    Analysing mesh-triangles-to-grid remapping component tests...')
disp('')

foldername_results = [foldername_automated_testing '/component_tests/results/remapping/mesh_triangles_to_grid'];
...
function analyse_remapping_test( filename_short)
  ...
  write_to_scoreboard_file( filename_short, mesh, d_mesh_ex, grid, d_grid_ex, d_grid);
...
function write_to_scoreboard_file( filename_short, mesh, d_mesh_ex, grid, d_grid_ex, d_grid)
  ...
  single_run = initialise_single_test_run( test_name, ...
    'component_tests/remapping/mesh_triangles_to_grid');
  ...
  int_mesh = sum( d_mesh_ex .* mesh.TriA);
